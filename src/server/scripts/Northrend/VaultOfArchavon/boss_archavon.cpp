    ...
    struct boss_archavonAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
                ...
                switch(eventId)
                {
                    ...
                    case EVENT_IMPALE:
                        if (Unit* target = me->getVictim())
                        {
                            ...
                            DoCast(target, SPELL_IMPALE); //apply damage
                            DoCast(target, 58672, true); //enter vehicle + stun
            ...
            if (impaleGuid)
            {
                if (impaleTimer < diff)
                {
                    if (Unit* target = me->GetUnit(*me, impaleGuid))
                        if (target->isAlive() && target->HasAura(58672))
                        {
                            target->RemoveAurasDueToSpell(58672);
                            target->ExitVehicle();
                        }
                    impaleTimer = 8000;
                    impaleGuid = 0;
