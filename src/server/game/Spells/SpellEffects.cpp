    ...
    if (unitTarget && unitTarget->isAlive())
    {
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_PALADIN:
            {
                ...
                {
                    float bv=m_caster->GetShieldBlockValue();
                    if(bv>2760) bv =2760 ;
                    damage += int32(bv * 0.5f);
                }
				// Judgement
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 8213:                                 // Savory Deviate Delight
                    ...
                    if (unitTarget->HasAura(8219))
                        unitTarget->RemoveAurasDueToSpell(8219);
                    if (unitTarget->HasAura(8220))
                        unitTarget->RemoveAurasDueToSpell(8220);
                    if (unitTarget->HasAura(8221))
                        unitTarget->RemoveAurasDueToSpell(8221);
                    if (unitTarget->HasAura(8222))
                        unitTarget->RemoveAurasDueToSpell(8222);

                    if (unitTarget->getGender() == 0)
                        m_caster->CastSpell(unitTarget, roll_chance_i(50) ? 8219 : 8221, true);
                    else
                        m_caster->CastSpell(unitTarget, roll_chance_i(50) ? 8220 : 8222, true);
                    return;
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            case 49560: // Death Grip
                ...
                if (Unit *unit = unitTarget->GetVehicleBase()) // Deathgrip shouldnt be allowed to cast on vehicles
                return;
                else
            ...
            switch (properties->Type)
            {
                ...
                case SUMMON_TYPE_TOTEM:
                {
                    ...
                    if (damage)                                            // if not spell info, DB values used
                    {
                        ...
                        {
                             summon->SetMaxHealth(m_caster->GetMaxHealth()*0.1f);
                             summon->SetHealth(m_caster->GetMaxHealth()*0.1f);
                        }
                        else
                        {
