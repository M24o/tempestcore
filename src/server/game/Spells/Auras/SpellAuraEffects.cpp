        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (HotWSpellId)
            {
                ...
                for (Unit::AuraEffectList::const_iterator i = mModTotalStatPct.begin(); i != mModTotalStatPct.end(); ++i)
                {
                    ...
                    {
                        int32 HotWMod = (*i)->GetSpellProto()->EffectBasePoints[1] * 1.5625;
...
void AuraEffect::HandleAuraModResistanceExclusive(AuraApplication const * aurApp, uint8 mode, bool apply) const
{
    ...
    for (int8 x = SPELL_SCHOOL_NORMAL; x < MAX_SPELL_SCHOOL; x++)
    {
        if (GetMiscValue() & int32(1<<x))
        {
            ...
            if(amount < GetAmount())
            {
                target->HandleStatModifier(UnitMods(UNIT_MOD_RESISTANCE_START + x), BASE_VALUE, 
                    float(GetAmount() - amount), apply);
                if (target->GetTypeId() == TYPEID_PLAYER)
                    target->ApplyResistanceBuffModsMod(SpellSchools(x),aurApp->IsPositive(),
                        float(GetAmount() - amount), apply);
            }
        }
    }
