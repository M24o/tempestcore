...
SpellMissInfo Unit::MagicSpellHitResult(Unit *pVictim, SpellEntry const *spell)
{
    ...
    if (!(spell->AttributesEx4 & SPELL_ATTR_EX4_CANT_BE_RESISTED))
    {
   ...
   // Roll chance
    ...
    if (pVictim->HasInArc(M_PI, this) || pVictim->HasAuraType(SPELL_AURA_IGNORE_HIT_DIRECTION))
    {
        int32 deflect_chance = pVictim->GetTotalAuraModifier(SPELL_AURA_DEFLECT_SPELLS) * 100;
        tmp += deflect_chance;
        // don't let chaos bolt get deflected
        if (spell->SpellIconID != 3178)
        {
           if (rand < tmp)
               return SPELL_MISS_DEFLECT;
...
//                    UNTESTED
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (dummySpell->Id == 49194)
            {
                ...
                uint32 ticks = durationInfo->Duration[0] / spellInfo->EffectAmplitude[EFFECT_0];
			
                basepoints0 = triggerAmount * damage / 100;
                // Glyph of Unholy Blight
                if (AuraEffect * glyph = GetAuraEffect(63332, EFFECT_0))
                ...
                for (Unit::AuraEffectList::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
                {
                     if ((*itr)->GetCasterGUID() != GetGUID() || (*itr)->GetId() != 50536)
                         continue;
